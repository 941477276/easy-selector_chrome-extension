...
function bottomUpSearch (a, b, c) {
  ...
  for (; f && f !== config.root.parentElement;) {
    ...
    if (b === Limit.All) {
      ...
      a = a.slice(0, 1), h && (a = a.concat(a.filter(dispensableNth).map(a => nthChild(a, h))));
    } else if (b === Limit.One) {
...
function attr (a) {
  ...
  return b.map(a => ({
    name: '[' + cssesc(a.name, { isIdentifier: !0 }) + '="' + cssesc(a.value) + '"]',
    penalty: .5
  }));
}
